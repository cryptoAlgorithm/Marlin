...
void DFRobot_LCD::init() {
	Wire.begin((int) _i2c_sda, _i2c_scl, 200000UL);
...
void DFRobot_LCD::createChar(uint8_t location, uint8_t charmap[]) {
...
void DFRobot_LCD::setColor(uint8_t color) {
    if (color >= sizeof(color_define)/sizeof(color_define[0])) return;
